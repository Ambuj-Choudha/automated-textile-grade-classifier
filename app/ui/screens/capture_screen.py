...
def _render_capture_column(col, mode, motor_ip, motor_port, selected_grades=None):
    ...
    with col:
        ...
        sample_number = st.text_input(t("sample_number"), value=st.session_state.get(f"last_sample_number_{tag}", ""), key=f"k_sample_number_{tag}", help="e.g., 00001, 00002, etc.") or ""
        stage_number  = st.text_input(t("stage_number"),  value=st.session_state.get(f"last_stage_number_{tag}",  ""), key=f"k_stage_number_{tag}",  help="e.g., 125, 500, 1000, etc.") or ""
        trial_number  = st.text_input(t("trial_number"),  value=st.session_state.get(f"last_trial_number_{tag}",  ""), key=f"k_trial_number_{tag}",  help="e.g., 1, 2, 3, etc.") or ""
        ...
        if is_grading:
            load_weight = st.text_input(t("load_weight"), value=st.session_state.get("last_load_weight_g", ""), key="k_load_weight_g", help="e.g., 155, 415, etc.") or ""
            ...
            for name, label_key, err_key, last_key, widget_key in _TRAINING_GRADE_INPUTS:
                if name not in (selected_grades or ()):
                    continue
                val = st.text_input(t(label_key), value=st.session_state.get(last_key, ""), key=widget_key, help="e.g., 1.5, 2, 2.5, etc.") or ""
